...
const TeachersRequests = () => {
    ...
    const handleApprove = async (request) => {
        const res = await axiosSecure.patch(`/teachers-requests/approve/${request._id}`, { email: request.email });
    ...
    return (
        <div className="overflow-x-auto w-full">
            <h2 className="text-2xl font-bold mb-6 text-slate-800">Total Requests: {requests.length}</h2>
                        ...
                        <tr key={request._id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-6">
                                <div className="flex items-center gap-4">
                                    <img src={request.image} className="w-12 h-12 rounded-2xl object-cover ring-2 ring-white shadow-sm" alt="" />
                                    <div>
                                        <div className="font-bold text-slate-900">{request.name}</div>
                                        <div className="text-xs text-slate-400 font-medium">{request.email}</div>
                                    </div>
                                </div>
                            </td>
                            {/* Details */}
                            <td className="px-6 py-6">
                                <div className="font-semibold text-slate-700 text-sm">{request.title}</div>
                            ...
                            <td>
                                <span className={`badge border-none font-bold text-[10px] ${request.status === 'approved' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'
                                    }`}>
                                    {request.status}
                                </span>
                            </td>
